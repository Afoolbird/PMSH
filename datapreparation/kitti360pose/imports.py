...
class Object3d:
    ...
    @classmethod
    def create_padding(cls):    # 在实例太少的情况下，我们填充了10个具有黑色和随机点坐标接近于零的点的虚拟实例，以便它们容易被模型识别。
...
class DescriptionBestCell:
    ...
    def from_matched(
        ...
        d.object_id = object_id     # 针对的是当前这个cell中的object_id(存放的所有object当中的index)
        ...
        d.best_offset_center = best_offset_center[0:2]  # Updated to best-cell  # 针对这个描述的位置以及当前的object实例中心的offset
...
class Cell:
    def __init__(self, idx, scene_name, objects: List[Object3d], cell_size, bbox_w):
        ...
        self.bbox_w = bbox_w  # Original pose in world-coordinates  整个空间的两个对角顶点
