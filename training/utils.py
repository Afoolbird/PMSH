...
def plot_retrievals(top_retrievals, dataset, count=100, top_k=3, green_thresh=10, path=""):
    ...
    while count_plotted < count:
        ...
        cv2.imwrite(f"{path}/ret_{count_plotted}_{pose.cell_id}.png", np.vstack(images_semantic_rgb))
        print(f"{pose.cell_id}" + "Saved pos!")
        count_plotted += 1
