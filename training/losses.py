...
# added by mll
# because calc_pose_error2 is computing the L2 distance between the offsets of pose's best cell and predicted output
# but this article has implement PMC strategy which makes this computing wrong.
def calc_pose_error3(
    gt_offsets, offsets=None
):
    errors = []
    batch_size = len(gt_offsets)
    for i_sample in range(batch_size):
        errors.append(np.linalg.norm(gt_offsets[i_sample] - offsets[i_sample]))
    return np.mean(errors)

def calc_pose_error4(
    gt_offsets, offsets=None
):
    errors = []
    batch_size = len(gt_offsets)
...
class ContrastiveLoss(torch.nn.Module):
    ...
    def forward(self, im, s):  # Norming the input (as in paper) is actually not helpful
        ...
        im = im / torch.norm(im, dim=1, keepdim=True)   # bs,256
        s = s / torch.norm(s, dim=1, keepdim=True)      # bs,256

        similarity = torch.mm(im, s.transpose(1, 0).contiguous())   # bs,bs
