...
class Kitti360BaseDataset(Dataset):
    def __init__(self, base_path, scene_name):
        ...
        self.cells = pickle.load(
            open(osp.join(base_path, "cells", f"{scene_name}.pkl"), "rb")   # submap
        ...
        self.poses = pickle.load(open(osp.join(base_path, "poses", f"{scene_name}.pkl"), "rb")) # list
        # 同一个poses可以有多个描述  至多3个  论文中说3个 但并不完全对应,会去除重复六个同样object_instance的描述
