...
class Kitti360CoarseSemiDataset(Kitti360BaseDataset):
    def __init__(
        ...
        if self.exclude:
            none_indexes = [index for index, (key, value) in enumerate(self.semi_cell.items()) if value is None]
            keys = list(self.semi_cell.keys())
            for index in sorted(none_indexes, reverse=True):    # 需要从后往前删除以保持index的正确性
                self.poses.pop(index)
                self.hint_descriptions.pop(index)
                del self.semi_cell[keys[index]]
    ...
    def __getitem__(self, idx):
        ...
        if semi_cell_id != None:
            ...
            semi_object_points = batch_object_points(semi_cell.objects, self.transform)
        else:
            semi_cell = cell
        ...
        return {
            ...
            "semi_objects": semi_cell.objects,
...
#### 这个排除掉了为None的半正样本 ####
class Kitti360CoarseSemiDataset2(Kitti360BaseDataset):
    def __init__(
        self,
        base_path,
        scene_name,
...
class Kitti360CoarseSemiDatasetMulti(Dataset):
    def __init__(
        ...
        Args:
            ...
            sample_close_cell (bool, optional): Sample any close-by cell per pose instead of the original one. Defaults to False.
        """
        self.exclude = False                # 这个参数代表是否排除为半正样本为None的数据
        ...
        self.sample_close_cell = sample_close_cell
        self.datasets = [
            # Kitti360CoarseSemiDataset2(       # 选择这个就要选择287行的代码   result = dataset[idx_in_dataset]
        ...
        if self.exclude:
            ...
            semi_uncertaintys = np.loadtxt(record_path, delimiter=',', dtype=float)[:, 1]
            ...
            self.semi_uncertaintys = np.delete(semi_uncertaintys, none_indexes)
    ...
    def __getitem__(self, idx):     # 相当于分别从相应的5个区域中获取到相应的位置信息
        ...
        for dataset in self.datasets:
            ...
            if idx_in_dataset < len(dataset):
                # result = dataset[idx_in_dataset] 
                # result.update({"semi_uncertainty": self.semi_uncertaintys[idx]})
                # return result
    ...
    def reset(self,):
        self.samples = [index for index in range(len(self.all_poses))]
