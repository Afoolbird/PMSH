...
class CellRetrievalNetwork(torch.nn.Module):
    ...
    def encode_objects(self, objects, object_points):
        ...
        batch = torch.tensor(batch, dtype=torch.long, device=self.device)   # [0,...,0,...,batch_size,...,batch_size]

        embeddings, pos_postions = self.object_encoder(objects, object_points)  # 每个batch中所有cell中的obj_instance的总和,256

        object_size = self.object_size  # 最大object实例的数量？ = 28

        index_list = [0]
        last = 0
        
        x = torch.zeros(len(objects), object_size, self.embed_dim).to(self.device)  # bs,28,256
        ...
        for idx in range(len(index_list) - 1):
            ...
            num_object = num_object_raw if num_object_raw <= object_size else object_size   # 控制每个cell中的obj_instance数量最多不超过28，不足的为0
            x[idx,: num_object] = embeddings[start : (start + num_object)]
            
        
        x = x.permute(1, 0, 2).contiguous() # 28,bs,256
        for idx in range(len(self.obj_inter_module)):
            x = self.obj_inter_module[idx](x)

        del embeddings, pos_postions
        
        x = x.max(dim = 0)[0]   # 28,bs,256 -> bs,256    相当于获取每个cell中的28个obj_instance中最大的那个特征
        x = F.normalize(x)
