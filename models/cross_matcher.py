...
class CrossMatchSemi(torch.nn.Module):
    def __init__(
        ...
        """
        super(CrossMatchSemi, self).__init__()
        ...
        self.mlp_sigmas = get_mlp_offset([self.embed_dim * 6, self.embed_dim * 6// 2, 3])
        ##### 正定和半正的MLP 分开 #####
        self.mlp_offsets_semi = get_mlp_offset([self.embed_dim, self.embed_dim // 2, 2])
        self.mlp_sigmas_semi = get_mlp_offset([self.embed_dim * 6, self.embed_dim * 6// 2, 3])
        ##### 正定和半正的MLP 分开 #####
    ...
    def forward(self, objects, hints, object_points, sign_semi=False):
        ...
        hint_encodings = self.language_encoder(hints)   # bs,6,128

        """
        3D submap branch
        """
        out = self.object_encoder(objects, object_points)   # bs*16,128
        ...
        object_encodings = object_encodings.reshape((batch_size, num_objects, self.embed_dim))  # bs,16,128
        ...
        if self.training:
            if sign_semi:
                desc1 = hint_encodings.transpose(0, 1).repeat(1, 2, 1)  # [num_hints, B, DIM]
            else:
                desc1 = hint_encodings.transpose(0, 1)
        else:
            desc1 = hint_encodings.transpose(0, 1)
        ...
        if self.cross_objects is not None:
            if len(self.cross_hints) == len(self.cross_objects):
                for i in range(len(self.cross_hints)):
                    desc0 = self.cross_objects[i](desc0, desc1) # [num_obj, B, DIM]
                    desc1 = self.cross_hints[i](desc1, desc0)   # [num_hints, B, DIM]
            else:
        ...
        desc2 = desc1.max(dim=0)[0]         # 6,2*bs,128 -> 2*bs,128
        offsets = self.mlp_offsets(desc2[:bs])   # 2*bs,128 -> 2*bs,2 (x,y)

        sigmas = self.mlp_sigmas(desc1.permute(1, 0, 2)[:bs].reshape(bs, -1))    # 2*bs,6*128    -> 2*bs,3
        ##### 正定和半正的MLP 分开 #####
        if sign_semi:
            offsets_semi = self.mlp_offsets_semi(desc2[bs:])
            sigmas_semi = self.mlp_sigmas_semi(desc1.permute(1, 0, 2)[bs:].reshape(bs, -1))
            offsets = torch.cat((offsets, offsets_semi), dim=0)
            sigmas = torch.cat((sigmas, sigmas_semi), dim=0)
        ##### 正定和半正的MLP 分开 #####
        update_y = Variable(torch.zeros((batch_size, 3)).float()).to(self.get_device())
        update_y[:, 1] = sigmas[:, 1]
        update_y[:, 0] = self.elu_plus(sigmas[:,0])
        update_y[:, 2] = self.elu_plus(sigmas[:,2])

        return offsets, update_y
