...
class ObjectEncoder(torch.nn.Module):
    def __init__(self, embed_dim: int, known_classes: List[str], known_colors: List[str], args):
        ...
        # 索引为 0 的类别对应的嵌入向量将始终为零，并且在训练过程中不会进行更新
        ...
        if args.pointnet_features == 0:
            ...
            self.mlp_pointnet = get_mlp([self.pointnet.dim2, self.embed_dim])       # 256, 256 or 128
        self.mlp_merge = get_mlp([len(args.use_features) * embed_dim, embed_dim])   # 4*(256 or 128), 256 or 128
    ...
    def forward(self, objects: List[Object3d], object_points):
        ...
        if ("class_embed" in self.args and self.args.class_embed) or (      # False
        ...
        if "class_embed" not in self.args or self.args.class_embed == False:    # True
            # Void all colors for ablation
            if "color" not in self.args.use_features:   # False
            ...
            object_features = [
                ...
                for pyg_batch in object_points  
            ]  # [B, obj_counts, PN_dim]  batch中每个数据单独处理
        ...
        if "color" in self.args.use_features:
            if "color_embed" in self.args and self.args.color_embed:
                ...
                #     torch.tensor(colors, dtype=torch.float, device=self.get_device())   # [total_objects, 3]
                # )
                color_embedding = self.color_encoder(
                    torch.tensor(np.array(colors), dtype=torch.float, device=self.get_device())   # [total_objects, 3]
                )
                embeddings.append(F.normalize(color_embedding, dim=-1))     # [total_objects, PN_dim]
        ...
        if "num" in self.args.use_features:
            num_points = []
            for objects_sample in objects:
                num_points.extend([len(obj.xyz) for obj in objects_sample]) # object_instance的点数
