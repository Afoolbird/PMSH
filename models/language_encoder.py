...
class LanguageEncoder(torch.nn.Module):
    ...
    def forward(self, descriptions):
        ...
        inputs = self.tokenizer(split_union_sentences, return_tensors="pt", padding = "longest")    # 根据最长的补齐
        shorten_sentences_indices = inputs["input_ids"] # 每个值代表一个词对应的词表索引
        attention_mask = inputs["attention_mask"]       # 哪些位置是补齐的(0)，哪些位置是真正的文本(1)。

        shorten_sentences_indices = shorten_sentences_indices.to(self.device)   # bs*6(句子总数),15
        attention_mask = attention_mask.to(self.device)
        out = self.llm_model(input_ids = shorten_sentences_indices, 
                        attention_mask = attention_mask,
                        output_attentions = False)
        description_encodings = out.last_hidden_state   # 编码特征  bs*6,15,1024
        ...
        for idx in range(len(self.intra_module)):
            description_encodings = self.intra_module[idx](description_encodings)   # 处理的是15,bs*6,1024 相当于处理的是每个句子内部(15)的关系
        description_encodings = description_encodings.permute(1,0,2).contiguous()
        description_encodings = description_encodings.max(dim = 1)[0]       # bs*6,15,1024  ->  bs*6,1024

        description_encodings = self.inter_mlp(description_encodings)
        description_encodings = description_encodings.view(batch_size, num_sentence, -1)    # bs,6,256
        
        if self.is_fine:
            return description_encodings
        
        description_encodings = description_encodings.permute(1,0,2)            # 6,bs,256 相当于处理的是每个句子之间(6)的关系
        for idx in range(len(self.inter_module)):
            description_encodings += self.inter_module[idx](description_encodings)
        
        description_encodings = description_encodings.max(dim = 0)[0]       # 6,bs,256  ->  bs,256
        return description_encodings
